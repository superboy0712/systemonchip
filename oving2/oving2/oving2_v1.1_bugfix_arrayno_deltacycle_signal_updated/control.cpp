...
void Controller::write(int id)
{
	...
	static int i = 0;//restores the current i th try with previous i-1 times successful push
			if (i > 8) i = 0; // successful 9 times, from beginning
			if(X[i]==id_temp){
				writep->LightControl(id_temp);// light on the id button, add offset 0xff represents sent from controller
				i ++;// i th try matches, move to next one.
			} else {
				writep->LightControl(0xffff); // 0xffff code reserved. means all light off.
